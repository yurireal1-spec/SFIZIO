...
def read_product_by_slug(
    ...
    """
    product = db.query(models.product.Product).filter(models.product.Product.slug == slug).first()
    if not product:
        raise HTTPException(status_code=404, detail="Produto não encontrado")
    return product
...
def read_categories(
    ...
    """
    categories = db.query(models.product.Category).offset(skip).limit(limit).all()
    return categories
